...
//qqjMark       这堆从~/ProCode/Pur/PurchaseGuide2_qqj.aspx  复制而来，没有修改！
public partial class ProCode_DMC_DcmGuide2_qqj : BasePage, IRequiresSessionState
{
    ...
    private const string _transNo = "DY";
...
");
        ...
        DBHelper.GetData(builder.ToString(), GetParams_qqj(needbegindate, needenddate, salebegindate, saleenddate, branchNo, itemClsno, itemBrand, supcustNo));
            ...
            from t_im_stock_target target where  target.item_no =c.item_no and target.branch_no like left(b.d_branch_no,4)+'%' ),0),               
           	(SELECT  isnull(sum(isnull(d.so_qty + d.pos_qty - d.pos_ret_qty+ d.pos_send_qty,0)),0)
                ...
                WHERE b.sheet_no=c.sheet_no and a.item_stock='1' and    
            	     ...
            	     and(convert(char(10),b.work_date,20) between @needbeginDate and @needendDate  )   
             		 and b.d_branch_no  like left(@branchno,4)+'%'   
                     and a.main_supcust  like left(@supcustNo,4)+'%'
                     and (c.item_no not in (select item_no from t_im_branch_stock where branch_no like left(b.d_branch_no,4)+'%' and len(branch_no)=6))                               
             group by c.item_no, a.item_no , a.unit_no, a.item_size,                            
